...
### Overview
Open WebUI is a user-friendly, self-hosted web interface for interacting with Large Language Models (LLMs). It provides a ChatGPT-like experience with support for various LLM providers.

### Container Configuration
```
open-webui-{id} - Web application (Python/FastAPI)
```

### Network Configuration
- **Network:** `n0o8c8wwko4880co4k0wcwc8` (isolated bridge)
- **Internal Port:** 8080
- **External Access:** Via Traefik (HTTPS only)

### Key Features
- **Multi-Model Support:** OpenAI, Anthropic, local Ollama, etc.
- **Chat History:** Persistent conversation storage
- **User Management:** Multi-user support with authentication
- **Model Settings:** Temperature, max tokens, system prompts
- **File Uploads:** Support for document analysis
- **API Integration:** Connect to various LLM providers

### Access
- **URL:** `https://ui.example.com`
- **Authentication:** Built-in user system
- **First User:** Admin account created on first visit

### Environment Variables
```bash
# Ollama integration (if using local Ollama)
...
### Storage
- **Data Directory:** `/app/backend/data` (Docker volume)
- **Database:** SQLite (included in data volume)
- **User Data:** Conversations, settings, uploaded files
...
services:
  open-webui:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.openwebui.rule=Host(`ui.example.com`)
...
```bash
# View logs
docker logs open-webui-{id} -f

# Restart service
docker restart open-webui-{id}

# Backup data
docker run --rm -v open-webui-data:/data \
  -v $(pwd):/backup alpine tar -czf /backup/openwebui-backup.tar.gz /data

# Restore data
docker run --rm -v open-webui-data:/data \
  -v $(pwd):/backup alpine tar -xzf /backup/openwebui-backup.tar.gz -C /
```
